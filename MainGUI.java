    ...
    private void addSession() {
        try {
            ...
            */
            ...
            Session s1 = new Session(id, topic, mentor, department, date, time, location, max);
            if(list==null){
                list = LinkedList.addFirst(s1, list);
            }else {
                Session firstSession = LinkedList.min(list);
                Session lastSession = LinkedList.max(list);

                if (s1.getSessionID() > firstSession.getSessionID()) {
                    if (s1.getSessionID() < lastSession.getSessionID()) {
                        list = LinkedList.insertAfter(s1, list);
                    }
                    list = LinkedList.addLast(s1, list);
                } else if (s1.getSessionID() < lastSession.getSessionID()) {
                    list = LinkedList.addFirst(s1, list);
                }
            }
            ...
            clearFields();
            IO.println("added items: " + list.toString());
        }
        catch(Exception e) {
